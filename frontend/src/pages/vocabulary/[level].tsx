                ...
                {!selectedTopic && (
                    <div className="w-full max-w-6xl mx-auto px-4 py-10">
                        ...
                        <div className="mb-12 animate-in slide-in-from-top-8 fade-in duration-700">
                            ...
                            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white p-10 lg:p-14 shadow-2xl group">
                                ...
                                <div className="absolute inset-0 opacity-10">
                                    ...
                                    <div className="absolute bottom-10 left-10 w-40 h-40 bg-white rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000"></div>
                                </div>
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <Badge className="bg-yellow-400/90 text-yellow-900 border-yellow-500/30 px-4 py-1.5 text-sm font-bold">
                                            {data.topics.reduce((sum, t) => sum + t.words.length, 0)} 단어
                                        </Badge>
                                    </div>
                        ...
                        {/* Topic Cards Grid with Staggered Animation */}
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {/* Special Card: My Vocabulary */}
                            <Card
                                className="cursor-pointer hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-2 border-yellow-200 bg-gradient-to-br from-yellow-50 to-orange-50 group relative overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-500 delay-100 fill-mode-backwards"
                                onClick={handleSelectSaved}
                            >
                                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
                                    <Star className="w-32 h-32 text-yellow-500 fill-yellow-500 group-hover:rotate-12 transition-transform duration-500" />
                                </div>
                                <CardHeader className="flex flex-row items-center gap-4 relative z-10 p-6">
                                    <div className="text-5xl bg-yellow-100 p-4 rounded-2xl group-hover:bg-yellow-200 group-hover:scale-110 transition-all shadow-md">
                                        ⭐
                                    </div>
                                    <div className="flex-1">
                                        <CardTitle className="text-xl mb-2 text-yellow-900 group-hover:text-yellow-700 transition-colors font-bold">
                                            내 단어장
                                        </CardTitle>
                                        <CardDescription className="flex items-center gap-2 text-yellow-700/80 font-medium">
                                            <Star className="w-4 h-4 fill-yellow-500 text-yellow-500" />
                                            <span className="text-base font-bold">{savedWords.length}</span> 저장됨
                                        </CardDescription>
                                    </div>
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity text-yellow-600">
                            ...
                            {data.topics.map((topic, idx) => (
                                <Card
                                    key={topic.id}
                                    className={cn(
                                        "cursor-pointer hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 hover:border-indigo-300 group bg-white",
                                        "animate-in slide-in-from-bottom-4 fade-in duration-500 fill-mode-backwards"
                                    )}
                                    style={{ animationDelay: `${(idx + 2) * 100}ms` }}
                                    onClick={() => handleSelectTopic(topic)}
                                >
                                    <CardHeader className="flex flex-row items-center gap-4 p-6">
                                        <div className="text-5xl bg-gradient-to-br from-gray-50 to-indigo-50 p-4 rounded-2xl group-hover:bg-gradient-to-br group-hover:from-indigo-100 group-hover:to-purple-100 group-hover:scale-110 transition-all shadow-sm">
                                            {topic.emoji}
                                        </div>
                                        <div className="flex-1">
                                            <CardTitle className="text-xl mb-2 group-hover:text-indigo-700 transition-colors font-bold">
                                                {topic.title}
                                            </CardTitle>
                                            <CardDescription className="flex items-center gap-2 font-medium">
                                                <BookOpen className="w-4 h-4 text-indigo-400" />
                                                <span className="text-base font-bold text-gray-700">{topic.words.length}</span> 단어
                                            </CardDescription>
                                        </div>
                        ...
                        <div className="w-full max-w-3xl">
                            ...
                            <div className="mb-8 p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-sm font-bold text-gray-700">학습 진행률</span>
                                ...
                                <Progress
                                    value={progress}
                                    className="h-3 bg-gray-100 [&>div]:bg-gradient-to-r [&>div]:from-indigo-500 [&>div]:to-purple-500 [&>div]:transition-all [&>div]:duration-500"
                                />
