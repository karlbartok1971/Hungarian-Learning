...
const GrammarLessonPage = () => {
  ...
  const fetchLesson = async () => {
    try {
      setLoading(true);
      // Ensure we hit the correct endpoint. ID is typically the DB ID (UUID) or slug.
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-lg border-red-100">
          ...
          <CardContent className="pt-6">
            <Button
              onClick={() => router.push('/grammar/a2')}
              variant="outline"
  ...
  return <GrammarLessonView lesson={lesson} level="A2" />;
};
